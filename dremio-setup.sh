...
apt-get -y update; apt-get -y install curl jq;
...
AUTH_RESPONSE=$(curl -s -X POST "http://dremio:9047/apiv2/login" \
...
curl -s -X PUT  "http://dremio:9047/apiv2/source/Samples" \
  -H "Content-Type: application/json" \
  -H "Authorization: _dremio$AUTH_TOKEN" \
...
curl -s -X PUT "http://dremio:9047/apiv2/source/Samples/folder_format/samples.dremio.com/NYC-taxi-trips" \
  -H "Content-Type: application/json" \
  -H "Authorization: _dremio$AUTH_TOKEN" \
